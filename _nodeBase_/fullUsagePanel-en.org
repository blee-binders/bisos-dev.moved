...
*** Create new snippets for file segments and new dblocks --
*** For cs-mu use csExamples.cs as template -- Identify new needed dblocks (b:py3:cs:file/controls + inMainExpected Symbols)
*** For cs-u use examples/parsArgsStdinCmndResult_csu.py -- Identify sections
*** Identify all old dblocks and modernize them
*** WAITING For templates get to 0 typing warnings -- Done in csExamples.cs not parsArgsStdinCmndResult_csu.py
*** Cleanup b. b.cs and b.io
*** Start with b_io, drive it with csExamples.cs
*** See if we can eliminate ast modules. Use log for file, line
*** Re-visit convertIcmToCs.sh
*** ===== P1:: All of above 1 week? =====  Due Sept 22, 2022
*** Revive encrypted file params with new CS
