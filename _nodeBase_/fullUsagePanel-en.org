...
** https://github.com/tomyjwu/python-microservices-with-grpc
...
*** B|
####+BEGIN: blee:bxPanel:foldingSection :outLevel 1 :sep t :title "Git-Shell Invoke-Perform Commands Services (GIPCS)" :anchor "" :extraInfo ""
* /[[elisp:(beginning-of-buffer)][|^]]  [[elisp:(blee:menu-sel:navigation:popupMenu)][Ξ]] [[elisp:(delete-other-windows)][|1]]/
* [[elisp:(show-all)][(>]]  _[[elisp:(blee:menu-sel:outline:popupMenu)][±]]_  _[[elisp:(blee:menu-sel:navigation:popupMenu)][Ξ]]_       [[elisp:(org-cycle)][| *Git-Shell Invoke-Perform Commands Services (GIPCS):* |]]    [[elisp:(org-shifttab)][<)]] E|
####+END
** +
** Sub-Title: Based On Expectations Complete Operations Services
...
**** --
**** aipx_inv --ipxBase=xxx -- anyIcm.py --par1 -i cmnd arg1 # no timeTags. base is ipxBase
**** aipx_inv --ipxRoot=xxx -- anyIcm.py --par1 -i cmnd arg1 # creates a timeTag and passes timeTag as insAsFPs
**** --
**** aipx_perf --ipxBase=xxx # Gethers everything as fileParams.
**** aipx_perf --ipxRoot=xxx # uses timeTags
**** -
*** Local Invoke-Perform Commands Services: LIPCS at a given Op Svc Multi-Unit AP (osmuAp)
**** +
**** osmuAp = Operations Service Multi-Unit Access Point
**** osmuAp maps to a git-repo (or a Bpo-Repo)
**** osmuAp has at its root: ./invs (invokations) ./state (for REST) ./files (root of FileSys) osmuSpec
**** --
**** lipcs_manage.py --bpoId=bpo --osmuAp=name -i create # creates osmupAP repos.
**** lipcs_manage.py --osmuApPath=path -i create # creates osmupAP repos.
**** --
**** lipcs_manage.py --osmuApPath=path -i inv -- anyOsmuIcm --par1 -i cmnd1 arg1
**** lipcs_manage.py --osmuApPath=path -i perf # searches in ./invs/
**** -
*** Remote Invoke-Perform Commands Services: RIPCS at a given Remote Op Svc Multi-Unit AP (rosmuAp)
**** +
